...
public class ImageUtil {
	...
	public final static String scaleByHeightAndWidth(String srcImageFile, String result, int height, int width,
			boolean bb, int l) {
		try {
	...
	public final static void pressImage(String pressImg, String srcImageFile, String destImageFile, int x, int y,
		...
		try {
			...
			g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, alpha));
			g.drawImage(src_biao, (wideth - wideth_biao) / x, (height - height_biao) / y, wideth_biao, height_biao,
					null);
			// 水印文件结束
			g.dispose();
	 ...
	 *            比例不对时是否需要补白：true为补白; false为不补白;
	 */
	public final static void imageProcessing(String pressImg, String source, String large, String scaleImageFile,
			String medium, String thumbnail, int mediumHeight, int mediumWwidth, int thumbnailHeight,
			int thumbnailWidth, int x, int y, float alpha, boolean bb, int l) {
		// 源图加水印处理
