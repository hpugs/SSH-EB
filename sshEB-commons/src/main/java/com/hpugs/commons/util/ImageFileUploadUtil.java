...
public class ImageFileUploadUtil {
	...
	public static String uploadImageFile(final byte[] imageBytes,final String imagePathFileName,final Map<Object,Object> fileUrlConfig,final String imageInfoPath){
	    if(Utils.objectIsEmpty(imageBytes)||Utils.objectIsEmpty(imagePathFileName)||Utils.objectIsEmpty(fileUrlConfig)||Utils.objectIsEmpty(imageInfoPath)){
		return null;
	    }
	    //上传文件的类型
	    String typeStr = imagePathFileName.substring(imagePathFileName.lastIndexOf("."));
	    //用于保存生成的新文件名
	    String newImagePathFileName = null;
	   //判断图片后缀是否符合要求，忽略大小写的判断
	    if(FileTypeUtil.checkIsImage(typeStr)){
		try{
		    //需要修改文件的后缀名称
		   ...
		   //定义新文件保存实际路径+配置文件systemConfig.properties中定义的文件生成目录规则：年月日
		    ...
		    FileUtils.writeByteArrayToFile(savefile,imageBytes);
		    newImagePathFileName = newImagePath + newName;		    
		}catch(Exception e){
		    e.printStackTrace();
		    return "图片上传失败!";
		}
	    }
	    return newImagePathFileName;
	}
	/**
	 * 上传图片文件拓展名，生成新的文件名。
	 * 
	 ...
	 *            获取配置文件systemConfig.properties中文件保存详细划分路径
	 * @param customerId
	 *            上传文件的customerId,用于生成一个唯一的用于该用户的路径
	 * @return 全新路径和文件地址
	 */
	public static String uploadImageFile(File imagePath,String imagePathFileName,Map<Object,Object> fileUrlConfig,String imageInfoPath,String customerId)  {
		if (imagePath != null) {
			// 1上传文件的类型
			String typeStr = imagePathFileName.substring(imagePathFileName.lastIndexOf("."));
			//判断图片后缀是否符合要求，忽略大小写的判断
