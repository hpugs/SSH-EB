 ...
 * @author 高尚
 * @version 1.0
 * @date 创建时间：2017年9月12日 下午5:24:58
 */
public class AliPushUtil {
	
	private static final String AccessKeyId = "";
	private static final String AccessKeySecret = "";
	private static final Long AppKey = 0L;

	/**
	 * @Description 开始推送
	 * @param target 推送目标: DEVICE:按设备推送    ALIAS:按别名推送    ACCOUNT:按帐号推送   TAG:按标签推送    ALL:广播推送
	 ...
	 * @param deviceType 设备类型 ANDROID iOS ALL.
	 * @param pushTitle 消息的标题
	 * @param pushBody 消息的内容
	 * @param popupTitle 
	 * @param popupBody
	 * @return 推送是否成功
	 *
	 ...
	 * @date 创建时间：2017年9月12日 下午5:53:00
	 */
	public static boolean push(final String target, final String targetValue, final String pushType, final String deviceType, final String pushTitle, final String pushBody, final String popupTitle, final String popupBody) throws ServerException, ClientException{
		boolean flag = true;
...
//        pushRequest.setTargetValue("ALL"); //根据Target来设定，如Target=DEVICE, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
        ...
        // 推送配置
        pushRequest.setTitle(pushTitle); // 消息的标题
        pushRequest.setBody(pushBody); // 消息的内容
        ...
        pushRequest.setIOSRemindBody(popupBody);//iOS消息转通知时使用的iOS通知内容，仅当iOSApnsEnv=PRODUCT && iOSRemind为true时有效
